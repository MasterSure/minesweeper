...
var time = document.getElementById('time'); //计时器
	var timer = setInterval(oTime,100) //定时器 100ms执行一次
...
Mine.prototype.init=function(){
	...
	this.parent.oncontextmenu=function(){    //取消浏览器自带的菜单
		return false;
	}
...
Mine.prototype.play=function(ev,obj){
	...
	if(oFlag.className=='setFlag'){     //如果当前插红旗按钮的状态为setFlag
		if(ev.which==1){
			//如果右击的是一个数字，那就不能点击
			if(obj.className && obj.className!='flag'){
				return;
...
Mine.prototype.gameOver=function(clickTd){
	...
	alert('游戏失败');
	clearInterval(timer); //游戏失败结束计时，清除定时器
}
...
btns[3].onclick=function(){  //点击改变状态
	// mine.init();
	oFlag.className=oFlag.className=='setFlag'?'':'setFlag';//切换CLASS
	alert(oFlag.className)
}
